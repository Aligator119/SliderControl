...
- (void)setInRangeTrackImage:(UIImage *)image {
	trackImageView.frame = CGRectMake(inRangeTrackImageView.frame.origin.x,trackImageView.frame.origin.y, max*(self.frame.size.width-2*self.frame.size.height)+self.frame.size.height, trackImageView.frame.size.height);
...
- (void)setTrackImage:(UIImage *)image {
	trackImageView.frame = CGRectMake(5, self.frame.size.height / 2.0 - 2 /*MAX((self.frame.size.height-image.size.height)/2.0,0)*/, self.frame.size.width-10, 4 /*MIN(self.frame.size.height,image.size.height)*/);
	trackImageView.image = image;
    trackImageView.layer.masksToBounds = YES;
    trackImageView.layer.cornerRadius = 2.0;
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
	...
	if (trackingSlider == minSlider) {
		...
		maxSlider.frame = CGRectMake(minSlider.frame.origin.x+self.frame.size.height+minimumRangeLength*(self.frame.size.width-self.frame.size.         height*2.0),
